    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                final int chunkSize = (n + tasks - 1) / tasks;
                final int left = (i * chunkSize) + 1; //left is inclusive
                int right = (left + chunkSize) - 1;   //right is inclusive
                if (right > n) right = n;

                for (int iter = 0; iter < iterations; iter++) {

                    //compute most left element of chunk
                    threadPrivateMyNew[left] = (threadPrivateMyVal[left - 1]
                            + threadPrivateMyVal[left + 1]) / 2.0;

                    //compute most right element of chunk
                    threadPrivateMyNew[right] = (threadPrivateMyVal[right - 1]
                            + threadPrivateMyVal[right + 1]) / 2.0;


                    //signal neighbors that left and right are ready
                    ph[i].arrive();

                    //compute internal elements of the chunk
                    ...
                    if(i > 0) ph[i-1].awaitAdvance(iter);
                    if(i < tasks - 1) ph[i+1].awaitAdvance(iter);
